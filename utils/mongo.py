        ...
        if code_data:
            if code_data["include_in_count"]:
                if code_data["uses"] == 0:
                    ...
                    user = user_data.find_one_and_update({"_id": user_id}, {"$inc": {"visits_counts.ALL.visits": 1, "visits_counts.ALL.unique_visits": 1, "visits_counts." + code_data["type"] + ".visits": 1, "visits_counts." + code_data["type"] + ".unique_visits": 1}, "$set": {"codes." + code: {"time": current_time, "visit_num": 1, "type": code_data["type"], "public_id": code_data["public_id"]}}}, projection={"codes." + code: True, "username": True, "public_id": True, "visits": True, "visits_counts.ALL": True})
                else:
                    type_data = visits_count.find_one_and_update({"_id": code_data["type"]}, {"$inc": {"visits": 1}})
                    visits_count.update({"_id": "ALL"}, {"$inc": {"visits": 1}})
                    user = user_data.find_one_and_update({"_id": user_id}, {"$inc": {"visits_counts.ALL.visits": 1, "visits_counts." + code_data["type"] + ".visits": 1}, "$set": {"codes." + code: {"time": current_time, "visit_num": code_data["uses"] + 1, "type": code_data["type"], "public_id": code_data["public_id"]}}}, projection={"codes." + code: True, "username": True, "public_id": True, "visits": True, "visits_counts.ALL": True})
